...
export default function Home() {
  ...
  const [filters, setFilters] = useState({ status: 'all', role: 'all', repo: 'all', search: '', page: 1 });
  ...
  const fetchIssues = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const uniqueRepos = [...new Set(data.issues.map(issue => issue.repository))];
  ...
  return (
    <div className="min-h-screen">
      <FilterBar filters={filters} setFilters={setFilters} repos={repos} />
      {error ? (
